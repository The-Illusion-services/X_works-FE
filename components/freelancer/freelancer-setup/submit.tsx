...
const SubmitDetails = ({
  ...
  useEffect(() => {
    ...
    // Load only the necessary data from localStorage
  ...
  // Get display data - prioritize localStorage but fall back to form data
  const displayData = {
    name: localStorageData.name || formData.fullname,
    professional_title: localStorageData.professional_title || formData.title,
    country: localStorageData.country || formData.country,
    city: localStorageData.city || formData.state,
    englishFluency: localStorageData.englishFluency || formData.englishFluency,
    date_of_birth: localStorageData.date_of_birth
      ? format(new Date(localStorageData.date_of_birth), 'yyyy-MM-dd')
      : '',
    rate:
      localStorageData.rate !== undefined
        ? localStorageData.rate
        : formData.rate,
    phone_number: localStorageData.phone_number || formData.phone,
    street_address: localStorageData.street_address || formData.address,
    zip_code: localStorageData.zip_code || formData.zipcode || '',
    skills: localStorageData.skills || formData.skills || [],
    bio: localStorageData.bio || formData.bio,
    Workcategory: localStorageData.Workcategory || formData.category,
    specialties: localStorageData.specialties || formData.specialities || [],
  };

  const isFormValid = () => {
    return (
      displayData.professional_title &&
      displayData.country &&
      displayData.zip_code &&
      displayData.Workcategory &&
      displayData.date_of_birth &&
      displayData.specialties &&
  ...
  // Add this function to your component or in a separate API service file
  const submitFreelancerProfile = async (displayData: LocalStorageData) => {
    try {
      ...
      const payload = {
        ...
        street_address: displayData.street_address,
        zip_code: displayData.zip_code,
        professional_title: displayData.professional_title,
        Workcategory: displayData.Workcategory,
      ...
      const response = await fetch(
        `https://x-works-be.onrender.com/api/accounts/freelancer/onboarding/`,
        {
          method: 'POST',
          headers: {
      ...
      console.error('Error submitting freelancer profile:', error);
      throw error;
    }
  };

  // Then update your handleSuccess function to use this API call:
  const handleSuccess = async () => {
    try {
      // Submit the data to the API
      await submitFreelancerProfile(displayData);
